...
export default class ButtonWebPart extends BaseClientSideWebPart<IButtonWebPartProps> {
  ...
  private handleChartClick = (chartType: string): void => {
    console.log(`Chart type selected: ${chartType}`);
    // Logic for handling chart click and routing or other actions
    // For SPFx and routing, you might use window.location or react-router's history.push
  };
  public render(): void {
    const element: React.ReactElement<IButtonProps> = React.createElement(
      Button,
      {
        ...
        onClick: this.handleChartClick // Make sure to add onClick handler here
      }
    );
